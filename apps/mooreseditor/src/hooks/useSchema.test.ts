...
describe("useSchema", () => {
  ...
  it("should load schema successfully", async () => {
    ...
    const { scanSchemaDirectory } = await import(
      "./useSchema/utils/schemaScanner"
    );
  ...
  it("should handle sample schema with definitions", async () => {
    const { getSampleSchema, getAllSampleSchemaMap } = await import(
      "../utils/devFileSystem"
    );
    const { scanSchemaDirectory } = await import(
      "./useSchema/utils/schemaScanner"
    ...
    const sampleSchemaMap = new Map([["blocks/ItemBlock", "blocks/ItemBlock"]]);
  ...
  it("should handle RefResolver properly", async () => {
    ...
    expect(mockResolver.debugBlocksSchema).toHaveBeenCalledWith(
      resolvedSchema,
      "items",
    );
  ...
  it("should handle missing sample schema in definitions", async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      "Sample schema blocks/MissingBlock not found",
    );
    expect(result.current.schemas.items).toEqual({ type: "object" });
  });
});
