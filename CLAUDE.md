...
```bash
# Tauriアプリとして起動（apps/mooreseditorから実行）
cd apps/mooreseditor && pnpm run tauri dev
```

### リントとタイプチェック

```bash
# リント（ルートから実行可能、turbo経由）
pnpm run lint

# タイプチェック（apps/mooreseditorから実行）
cd apps/mooreseditor && pnpm run type-check
```

### テストの実行

```bash
# ユニットテスト（apps/mooreseditorから実行）
cd apps/mooreseditor && pnpm run test
```
...
- `apps/mooreseditor/src/components/` - UIコンポーネント
- `apps/mooreseditor/src/hooks/` - カスタムフック
- `apps/mooreseditor/src/utils/` - ユーティリティ関数
- `apps/mooreseditor/src/libs/schema/` - スキーマ関連の型定義
- `apps/mooreseditor/src/nodeEditor/` - ノードグラフエディタ
