...
export function PrimitiveTypeInput({ showLabel = false, property, propertySchema, value, onChange }: Props) {
  ...
  } else if ('format' in propertySchema && propertySchema.format === '@autoGenerated') {
    return <>自動生成</>
  } else if ('format' in propertySchema && propertySchema.format.startsWith('@foreignKey')) {
    const [_format, schemaId, idPropName, labelPropName] = propertySchema.format.split(':')
