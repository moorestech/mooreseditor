  ...
  const duplicateItem = useCallback(
    (index: number) => {
      if (onDataChange && schema.items) {
        ...
        // IDを再生成した複製アイテムを作成
        const finalDuplicatedItem = regenerateIds(
          duplicatedItem,
          schema.items,
          newArray,
          index,
...
function regenerateIds(
  ...
  if (itemSchema.type === "object" && itemSchema.properties) {
    ...
    itemSchema.properties.forEach((prop) => {
      ...
      if (
        "type" in propSchema &&
        propSchema.type === "uuid" &&
        propSchema.autoGenerated
      ) {
        newItem[key] = generateUuid();
      }
      ...
      if (
        ...
        newItem[key] = newItem[key].map((arrayItem: any, idx: number) =>
          regenerateIds(
            arrayItem,
            (propSchema as ArraySchema).items,
            newItem[key],
            idx,
          ),
        );
