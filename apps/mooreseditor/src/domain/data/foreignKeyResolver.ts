import type { ForeignKeyConfig } from "../schema/types";
...
export class ForeignKeyResolver {
   ...
   */
  private expandAllPaths(
    pathPattern: string,
    collectHierarchy: boolean = false,
  ): PathResult[] {
    ...
    if (
      collectHierarchy &&
      this.config.hierarchyDisplayPaths &&
      this.config.hierarchyDisplayPaths.length > 0
    ) {
      // Use configured hierarchy paths
      hierarchyPaths.push(
        ...this.parseHierarchyPaths(this.config.hierarchyDisplayPaths),
      );
    }
          ...
          if (hierarchyPaths && hierarchyValues !== undefined) {
            ...
            for (const hierarchyPath of hierarchyPaths) {
              if (
                this.pathsMatch(
                  pathSoFar,
                  hierarchyPath.slice(0, pathSoFar.length),
                )
              ) {
   ...
   */
  private pairResults(
    idResults: PathResult[],
    displayResults: PathResult[],
  ): ForeignKeyOption[] {
    ...
    idResults.forEach((idResult) => {
      ...
      if (matchingDisplay) {
        ...
        if (
          matchingDisplay.hierarchyValues &&
          matchingDisplay.hierarchyValues.length > 0
        ) {
  ...
  private indicesMatch(
    indices1: Map<string, number>,
    indices2: Map<string, number>,
  ): boolean {
 ...
 */
export function validateForeignKeyPath(path: string): {
  valid: boolean;
  error?: string;
} {
  if (!path) {
  ...
  for (const segment of segments) {
    ...
    if (segment.includes("[")) {
      ...
      if (!arrayMatch || !arrayMatch[1]) {
        return {
          valid: false,
          error: `Invalid array notation in segment: ${segment}. Use format: name[*]`,
        };
      }
    }
  }
