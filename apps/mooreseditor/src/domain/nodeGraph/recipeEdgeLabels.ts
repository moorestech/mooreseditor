...
import type { ObjectPropertySchema, ObjectSchema } from "../schema/types";
...
export function buildSchemaRecordIndex(
  ...
  const dataObj = asJsonObject(column?.data);
  const rows = dataObj?.[meta.dataArrayPath];
...
export function buildForeignNameResolver(
  ...
  return (schemaId: string, guid: string): string | null => {
    ...
    if (!index) {
      ...
      if (meta?.guidField) {
        const column = jsonData.find((entry) => entry.title === schemaId);
        const colObj = asJsonObject(column?.data);
        const rows = colObj?.[meta.dataArrayPath];
