...
function CraftRecipeTable() {
  const [craftRecipes, setCraftRecipes] = useState<ReadonlyArray<CraftRecipe>>(
    Mod.instance ? Mod.instance.craftRecipeConfig.CraftRecipes : []
  )
          ...
          {craftRecipes.map((recipe, index) => {
            return (
              <CraftRecipeTableRow
                key={index}
                recipe={recipe}
                items={Mod.instance?.itemConfig.items ?? []}
                onSubmit={recipe => {
          ...
          <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
            <TableCell>
              ...
              <CraftRecipeEditModal
                isOpen={isNewCraftRecipeModalOpen}
                recipe={createTempRecipe()}
                items={Mod.instance?.itemConfig.items ?? []}
                onClose={() => {
