...
**Files:**
- Move: `packages/plugin-sdk/src/utils/createInitialValue.ts` → `packages/plugin-sdk/src/utils/initialization/createInitialValue.ts`
...
- Move: `packages/plugin-sdk/src/utils/switchFieldProcessor.ts` → `packages/plugin-sdk/src/utils/initialization/switchFieldProcessor.ts`
...
- Modify: `packages/plugin-sdk/src/hooks/useArrayDataManager.ts`
...
- [ ] **Step 1: Run the existing initialization tests as characterization coverage**

Run:

```bash
pnpm --filter @moorestech/mooreseditor-plugin-sdk exec vitest run src/utils/dataInitializer.test.ts src/utils/primitiveDefaultValue.test.ts src/utils/deepMerge.test.ts src/utils/schemaToZod.test.ts
```

Expected: all existing initialization tests pass.
...
- [ ] **Step 3: Move the responsibility groups with `apply_patch` and verify RED**

Move the listed initialization, object, and validation implementations and tests without changing behavior. Before repairing imports, run:

```bash
pnpm --filter @moorestech/mooreseditor-plugin-sdk exec vitest run src/utils/initialization/__tests__ src/utils/object src/utils/validation
```

Expected: FAIL with unresolved relative imports. The failure must be caused by the directory move, not by a changed assertion.

- [ ] **Step 4: Repair imports inside `utils/initialization`**

Use these exact imports, preserving every implementation body below them:

```ts
// initialization/createInitialValue.ts
...
// initialization/switchFieldProcessor.ts
import { DataInitializer } from "./dataInitializer";
...
The moved object and validation tests continue importing sibling implementations:
...
```ts
// packages/plugin-sdk/src/hooks/useArrayDataManager.ts
...
```ts
export * from "./utils/initialization/switchFieldProcessor";
export * from "./utils/initialization/dataInitializer";
export * from "./utils/initialization/createInitialValue";
...
- [ ] **Step 6: Run focused tests and prove no old internal import remains**

Run:

```bash
pnpm --filter @moorestech/mooreseditor-plugin-sdk exec vitest run src/utils/initialization/__tests__ src/utils/object src/utils/validation
rg -n 'utils/(createInitialValue|dataInitializer|primitiveDefaultValue|switchFieldProcessor|deepMerge|schemaToZod)' packages/plugin-sdk/src --glob '*.ts' --glob '*.tsx'
```

Expected: tests pass. `rg` reports only paths containing `utils/initialization/`, `utils/object/`, or `utils/validation/`; no consumer points at a removed path.

- [ ] **Step 7: Run the plugin-sdk suite, verify layout, and commit**
...
Expected: the package suite passes; direct file counts are `utils: 10`, `initialization: 4`, `initialization/__tests__: 3`, `object: 2`, `validation: 2`; both split tests are at most 200 lines.

Commit:

```bash
git add packages/plugin-sdk/src
git commit -m "refactor(plugin-sdk): group initialization utilities"
```
