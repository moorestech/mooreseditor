...
test.describe("Array Duplication Feature Tests", () => {
  test("Test array duplication functionality", async ({ page }) => {
    ...
    console.log(
      "CraftRecipes table loaded, now clicking Edit button to access requiredItems array...",
    );
    ...
    if (editButtonCount > 0) {
      ...
      console.log(
        "Form view loaded. Now looking for array fields with copy buttons...",
      );
      ...
      for (const selector of copySelectors) {
        ...
        if (count > 0) {
          ...
          const saveLogFound = consoleLogs.some(
            (log) =>
              log.includes("craftRecipes:") ||
              log.includes("データが保存されました"),
          );
      ...
      if (!copyButtonFound) {
        ...
        const arrayElements = page.locator(
          '[class*="array"], [class*="Array"], div:has(button:has-text("Add Item")), div:has(button:has-text("Delete"))',
        );
        ...
        for (let i = 0; i < Math.min(iconButtonCount, 10); i++) {
          ...
          console.log(
            `Button ${i}: Text="${buttonText}", HTML contains: ${buttonHTML.substring(0, 100)}...`,
          );
        }
      }
    } else {
