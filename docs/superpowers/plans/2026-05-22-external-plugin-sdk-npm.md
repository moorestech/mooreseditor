...
**Goal:** Make `@moorestech/mooreseditor-plugin-sdk` npm-publishable so mooreseditor plugins can be developed in a completely separate repository, and prove it works end-to-end via a local Verdaccio registry and a throwaway test repo.
...
- `SHARED_DEPENDENCIES` (`src/vite/sharedDeps.js`): `react`, `react-dom`, `react/jsx-runtime`, `@mantine/core`, `@mantine/hooks`, `@mantine/notifications`, `@tabler/icons-react`, `@xyflow/react`, `@moorestech/mooreseditor-plugin-sdk`.
- `scripts/check-plugin-contracts.ts` imports `SHARED_DEPENDENCIES` / `sharedDependencySpecs` from `@moorestech/mooreseditor-plugin-sdk/vite` and asserts host/sdk/node-graph dependency versions match.
- Host (`apps/mooreseditor`) imports `@moorestech/mooreseditor-plugin-sdk` in `App.tsx`, `pluginHost/loader.ts`, `pluginHost/hostApi.ts`, `pluginHost/usePlugins.ts`, `utils/dataValidator.ts`.
...
```bash
pnpm --filter @moorestech/mooreseditor-plugin-sdk add -D tsup@^8
```
...
```bash
cd packages/plugin-sdk && pnpm run build && pnpm pack && tar -tzf moorestech-mooreseditor-plugin-sdk-1.0.0.tgz
```
...
```bash
cd packages/plugin-sdk && mkdir -p /tmp/sdk-tarball-check && tar -xzf moorestech-mooreseditor-plugin-sdk-1.0.0.tgz -C /tmp/sdk-tarball-check && node -e "const p=require('/tmp/sdk-tarball-check/package/package.json'); console.log('exports[.]:', JSON.stringify(p.exports['.'])); console.log('exports[./vite]:', JSON.stringify(p.exports['./vite']))"
```
...
```bash
cd packages/plugin-sdk && rm -f moorestech-mooreseditor-plugin-sdk-1.0.0.tgz && rm -rf /tmp/sdk-tarball-check
...
Expected: exits 0, no error thrown. (This imports `@moorestech/mooreseditor-plugin-sdk/vite` via the unchanged source `exports`.)
...
```bash
pnpm --filter @moorestech/mooreseditor-plugin-sdk run type-check && pnpm --filter @mooreseditor/mooreseditor run type-check
```
...
```bash
pnpm --filter @moorestech/mooreseditor-plugin-sdk run test
```
...
// @moorestech/mooreseditor-plugin-sdk, etc.) and emits dist/index.js + plugin.json.
...
# Only the @moorestech scope is redirected; everything else uses the
# public npm registry.
@moorestech:registry=http://localhost:4873
```
...
Expected: `+ @moorestech/mooreseditor-plugin-sdk@1.0.0`. MUST use `pnpm publish` — `npm publish` ignores `publishConfig.exports` and would publish a broken package.
...
```bash
curl -s http://localhost:4873/@moorestech%2fmooreseditor-plugin-sdk | node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{const j=JSON.parse(s);const v=j.versions['1.0.0'];console.log('exports[.]:',JSON.stringify(v.exports['.']));console.log('exports[./vite]:',JSON.stringify(v.exports['./vite']));})"
```
...
- [ ] **Step 2: Point the @moorestech scope at Verdaccio**
...
Expected: `.npmrc` contains `@moorestech:registry=http://localhost:4873`.
...
Expected: exits 0. `node_modules/@moorestech/mooreseditor-plugin-sdk/package.json` exists with `version: 1.0.0` and `exports` pointing at `dist/`.
...
```bash
cd ~/WebstormProjects/mooreseditor-plugin-test && ls node_modules/@moorestech/mooreseditor-plugin-sdk && test -f node_modules/@moorestech/mooreseditor-plugin-sdk/dist/index.js && test ! -d node_modules/@moorestech/mooreseditor-plugin-sdk/src && echo "DIST-ONLY OK"
```
...
const shared=['react','react-dom','react/jsx-runtime','@mantine/core','@mantine/hooks','@mantine/notifications','@tabler/icons-react','@xyflow/react','@moorestech/mooreseditor-plugin-sdk'];
...
if(!m.includes('@moorestech/mooreseditor-plugin-sdk')) throw new Error('plugin-sdk was bundled, expected external');
...
- `@moorestech/mooreseditor-plugin-sdk` is now npm-publishable (verified via Verdaccio).
...
- **Follow-up requiring user approval (out of scope):** publishing `@moorestech/mooreseditor-plugin-sdk@1.0.0` to the real npm registry (`pnpm --filter @moorestech/mooreseditor-plugin-sdk publish`). Do NOT publish to real npm without explicit approval.
...
**Type consistency:** `pluginMetadata` shape (`id`/`name`/`version`) is consistent across `pluginMetadata.ts`, `plugin-entry.tsx`, and the `plugin.json` check in Task 8 Step 3. `PluginManifest`/`HostAPI`/`PluginView` imported from `@moorestech/mooreseditor-plugin-sdk` match the contract used by `node-graph/src/plugin-entry.tsx`. `mooresPlugin(pluginMetadata)` usage matches `node-graph/vite.config.ts`. Build entry path `src/plugin-entry.tsx` matches the hardcoded entry in `mooresPlugin.js`.
